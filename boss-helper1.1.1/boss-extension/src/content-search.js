...
(function () {
  ...
  const extLog = (text, level) => {
    chrome.runtime.sendMessage({ type: 'CONTENT_LOG', text: '🔍 [页面] ' + text, level: level || 'info' }).catch(() => {});
  };
  ...
  chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
    if (msg.type === 'SCRAPE') {
      scrape(msg.count || 20).then(jobs => sendResponse({ success: true, jobs })).catch(e => sendResponse({ success: false, error: e.message }));
      return true;
    }
    if (msg.type === 'OPEN_JD') {
    ...
    if (msg.type === 'GO_CHAT') {
      goChat(msg.job, msg.greeting, msg.cfg).then(r => sendResponse(r)).catch(e => sendResponse({ success: false, error: e.message }));
      return true;
    }
  });
})();
