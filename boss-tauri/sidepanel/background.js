...
// chromeStorageLocal 兼容层(IIFE 内自带,避免依赖 sidepanel.js 全局)
const chromeStorageLocal = {
  async get(keys, callback) {
    const keysArray = Array.isArray(keys) ? keys : (typeof keys === 'string' ? [keys] : (keys ? Object.keys(keys) : []));
    const data = await invoke('storage_get', { keys: keysArray });
    const res = typeof data === 'string' ? JSON.parse(data) : (data || {});
    if (callback) callback(res);
    return res;
  },
  async set(obj, callback) {
    await invoke('storage_set', { data: obj });
    if (callback) callback();
  },
  async remove(keys, callback) {
    const keysArray = Array.isArray(keys) ? keys : [keys];
    await invoke('storage_remove', { keys: keysArray });
    if (callback) callback();
  }
};
...
async function curUrl(tabId) {
  try {
    const url = await invoke('eval_in_boss', { script: 'window.location.href' });
...
async function sendToTab(tabId, msg) {
  ...
  if (msg.type === 'SCRAPE') {
    script = `(async () => {
      try {
        ...
        window.__bossResult = { success: true, jobs };
      } catch (e) {
        window.__bossResult = { success: false, error: e.toString() };
      }
    })();
    window.__bossResult = null;`;
  } else if (msg.type === 'OPEN_JD') {
    script = `(async () => {
      try {
        const r = await window.__bossSearch.openJD(${JSON.stringify(msg.job)});
        window.__bossResult = r || { success: true };
      } catch (e) {
        window.__bossResult = { success: false, error: e.toString() };
      }
    })();
    ...
    // goChat 会触发页面导航(点击"继续沟通"),导航后 window.__bossResult 会丢失
    // 所以不轮询,直接触发后返回成功,由调用方检测 URL 变化
    script = `(async () => {
      try {
        const r = await window.__bossSearch.goChat(${JSON.stringify(msg.job)}, ${JSON.stringify(msg.greeting)}, ${JSON.stringify(msg.cfg)});
        // 导航前把结果存到 sessionStorage(跨页面保留)
        try { sessionStorage.setItem('__bossGoChat', JSON.stringify(r || { success: true })); } catch(_) {}
      } catch (e) {
        try { sessionStorage.setItem('__bossGoChat', JSON.stringify({ success: false, error: e.toString() })); } catch(_) {}
      }
    })();`;
    try {
      await invoke('eval_in_boss', { script });
      // goChat 触发导航后不轮询 window(会丢失),直接返回成功
      return { success: true, navigated: true };
    } catch (e) {
      return { success: false, error: 'eval 失败: ' + String(e) };
    }
  ...
  if (script) {
    try {
      // 第一步:触发 async 操作(不等待返回值)
      await invoke('eval_in_boss', { script });
      // 第二步:轮询读取 window.__bossResult(200ms 间隔,快速响应)
      const deadline = Date.now() + 30000;
      while (Date.now() < deadline) {
        await new Promise(r => setTimeout(r, 200));
        const r = await invoke('eval_in_boss', { script: 'window.__bossResult ? JSON.stringify(window.__bossResult) : ""' });
        if (r && typeof r === 'string' && r.length > 0) {
          try { return JSON.parse(r); } catch (_) { return { success: false, raw: r }; }
        }
      }
      return { success: false, error: 'scrape 轮询超时(30s)' };
    } catch (e) {
      return { success: false, error: 'eval 失败: ' + String(e) };
    }
  }
...
async function runCollect() {
  try {
    console.log('[BG] runCollect 开始执行');
  ...
  for (const cityObj of cities) {
    ...
    console.log('[BG] 准备导航到:', url);
    const tab = await ensureTab(url);
    console.log('[BG] 导航完成, 准备注入脚本');
    await ensureInjected(tab.id, 'src/content-search.js');
    console.log('[BG] 脚本注入完成, 准备 scrape');

    const r = await sendToTab(tab.id, {
      type: 'SCRAPE',
      count: cfg.count || 20
    });
    console.log('[BG] scrape 返回:', JSON.stringify(r));
  ...
  emit('background-to-panel', { type: 'QUEUE_READY', queue: state.queue });
    ...
    console.error('[BG] runCollect 异常:', e.message, e.stack);
    log('收集流程异常: ' + e.message, 'error');
...
async function runDeliver() {
  ...
  for (let i = state.queueIndex; i < total; i++) {
    ...
    console.log('[BG] GO_CHAT 耗时:', Date.now() - t_go, 'ms, 结果:', JSON.stringify(r3));
    const chatUrlFromTab = r3 && r3.chatUrl;

    // 若 goChat 已触发导航(继续沟通),等待页面跳转完成
    let chatTab = null;
    if (r3 && r3.navigated) {
      // 已点击"继续沟通",等待跳转到聊天页
    ...
    console.log('[BG] SEND_ALL 耗时:', Date.now() - t_send, 'ms, 结果:', JSON.stringify(r2));
...
function initBackgroundListeners() {
  console.log('[BG] initBackgroundListeners 注册监听器');
  listen('panel-to-background', (event) => {
    const msg = event.payload;
    console.log('[BG] 收到消息:', JSON.stringify(msg));
    if (!msg) return;
