...
const invoke = async (cmd, args) => {
  ...
  console.warn('[Tauri IPC] core.invoke not ready for', cmd);
  return null;
};

const emit = async (evt, payload) => {
  if (window.__TAURI__ && window.__TAURI__.event) {
    return window.__TAURI__.event.emit(evt, payload);
  }
};

const listen = async (evt, handler) => {
  if (window.__TAURI__ && window.__TAURI__.event) {
    return window.__TAURI__.event.listen(evt, handler);
  }
  return () => {};
};

// chromeStorageLocal 兼容层
...
async function getClientId() {
  const data = await chromeStorageLocal.get('clientId');
...
// ── 载入/保存配置 ──
function loadCfg() {
  chromeStorageLocal.get(CFG_KEYS.concat(['imageResumeData']), d => {
...
async function saveCfg() {
  ...
  await chromeStorageLocal.set(obj);
}
...
$('btnCollect').addEventListener('click', async () => {
  console.log('[PANEL] 开始收集按钮被点击');
  await saveCfg();
  if (!$('keyword').value.trim()) return addLog('请先填写岗位关键词', 'error');
  setRunning(true);
  $('phaseText').textContent = '收集中...';
  addLog('开始收集 + 投递', 'info');
  console.log('[PANEL] 发送 START_COLLECT 消息');
  chromeRuntime.sendMessage({ type: 'START_COLLECT' });
});

$('btnPause').addEventListener('click', () => {
  if ($('btnPause').textContent === '暂停') {
    chromeRuntime.sendMessage({ type: 'PAUSE' });
    $('btnPause').textContent = '继续';
  } else {
    chromeRuntime.sendMessage({ type: 'RESUME' });
    $('btnPause').textContent = '暂停';
  }
});

$('btnStop').addEventListener('click', () => {
  chromeRuntime.sendMessage({ type: 'STOP' });
  setRunning(false);
  $('phaseText').textContent = '已停止';
});

$('btnReset').addEventListener('click', () => {
  chromeRuntime.sendMessage({ type: 'RESET' });
  setRunning(false);
...
$('btnDeliver').addEventListener('click', async () => {
  ...
  // 更新 SW 中的队列为选中岗位
  await chromeStorageLocal.set({ sw_queue: selected, sw_queue_index: 0 });
  chromeRuntime.sendMessage({ type: 'START_DELIVER' });
});
...
async function updateDashboard() {
  ...
  const data = await chromeStorageLocal.get(['jobTracker', dailyKey]);
...
$('logoutBtn').addEventListener('click', async () => {
  ...
  await chromeStorageLocal.remove(['sessionToken', 'clientId', 'email']);
...
async function loadAndRenderHistory() {
  const data = await chromeStorageLocal.get(['appliedHistory']);
...
(async () => {
  const { sessionToken } = await chromeStorageLocal.get('sessionToken');
  if (!sessionToken) {
    $('loginPanel').style.display = 'flex';
    $('mainPanel').style.display = 'none';
    return;
  ...
  try {
    ...
    const data = await resp.json();
    if (data.valid) {
      await chromeStorageLocal.set({ clientId: data.client_id });
      await showMainPanel(data.client_id);
    } else {
      await chromeStorageLocal.remove(['sessionToken', 'clientId']);
      $('loginPanel').style.display = 'flex';
      $('mainPanel').style.display = 'none';
    }
  } catch (e) {
...
async function renderVersionUI(versionInfo) {
  ...
  if (latest_version && semverCompare(CURRENT_VERSION, latest_version) < 0) {
    const { ignored_version } = await chromeStorageLocal.get('ignored_version');
...
if ($('closeUpdateBannerBtn')) {
  $('closeUpdateBannerBtn').addEventListener('click', async () => {
    ...
    if (version_info && version_info.latest_version) {
      await chromeStorageLocal.set({ ignored_version: version_info.latest_version });
    }
  });
}
